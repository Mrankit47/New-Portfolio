...
const Navbar = () => {
    ...
    return (
        <>
            <nav className={`fixed top-0 w-full z-[100] px-4 sm:px-6 md:px-24 py-4 sm:py-6 flex justify-between items-center transition-all duration-500 ${isScrolled ? 'bg-dark/80 backdrop-blur-xl border-b border-white/5 py-3 sm:py-4' : 'bg-transparent'}`}>
            ...
            {/* Mobile Menu Overlay — rendered outside nav so it covers everything */}
            <div className={`fixed inset-0 bg-[#0b0b0b] z-[110] flex flex-col items-start justify-start pt-24 pl-8 space-y-8 transition-all duration-500 md:hidden ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
